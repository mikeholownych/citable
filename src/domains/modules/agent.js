import { defineDomainModule } from "../interface.js";
import { AGENT_DETECTORS as detectors } from "../../detectors/agent.js";

export const agentDomainModule = defineDomainModule({
  module_id: "agent",
  namespace: "AGENT",
  title: "Agent Readiness & Protocol Surface",
  description: "llms.txt, MCP servers, A2A discovery, and Web Bot permissions.",
  conditions: detectors.filter((d) => d.namespace === "AGENT"),
  observation_kinds: ["mcp_transport"],
  collectors: {
    static_analysis: { method: "static_analysis" },
    synthetic_fetch: { method: "synthetic_fetch" },
  },
  report_projections: {
    summary: (determinations) => ({
      domain: "agent",
      total: determinations.length,
      passed: determinations.filter((d) => d.status === "PASS").length,
      failed: determinations.filter((d) => d.status === "FAIL").length,
      warning: determinations.filter((d) => d.status === "WARNING").length,
      not_tested: determinations.filter((d) => d.status === "NOT_TESTED").length,
    }),
  },
  gating: {
    default_enabled: true,
  },
});

export default agentDomainModule;
