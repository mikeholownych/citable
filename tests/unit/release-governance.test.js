...
function releaseFixtureRoot() {
  ...
  fs.writeFileSync(path.join(root, 'dist', 'universal', 'manifest.json'), JSON.stringify({ providers: { fixture: { files: 81 } } }));
  return root;
}
...
test('canonical release manifest is deterministic for fixed inputs and binds generated projections', () => {
  ...
  assert.equal(first.manifest.facts.distribution_files_per_provider, 81);
